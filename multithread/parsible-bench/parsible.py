...
class Parsible(object):
    ...
    def main(self, tasks):
        ...
        def parse_chunk(lines):
            for line in lines:
                parsed = self.parsing_function(line)
                #self.run_processors(parsed)

        lines = self.log_file.readlines()
        chunks = self.chunks(lines, tasks)
        fs = []
        for c in list(chunks):
            fs.append(Future(parse_chunk, c))
        [f() for f in fs]

        # # Abstract all the messy generator logic away into a simple for-each
        # parsed_log_file = list(parsed_log_file)
